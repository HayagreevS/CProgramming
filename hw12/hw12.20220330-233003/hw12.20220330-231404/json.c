...
static void append(Element val, Node** a_head, Node** a_tail) {
	Node* new_tail = malloc(sizeof(*new_tail));
	*new_tail = (Node) {.element = val, .next = NULL};
...
void free_element(Element element) {
	if(element.type == ELEMENT_STRING){
		...
		while(element.as_list != NULL) {
			...
			if(((element.as_list) -> element).as_string != NULL) {
...
bool parse_list(Node** a_head, char const** a_pos) {
	...
	if(count == -1) {
		...
		*a_pos = *a_pos + 1;
		Element element;
		found = parse_element(&element, a_pos);
		append(element, &head, &tail);
